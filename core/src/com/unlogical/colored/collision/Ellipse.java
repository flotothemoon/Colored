...
public class Ellipse extends Shape
{
	 ...
	 * 
	 */
	private static final long serialVersionUID = -2730154814767665607L;
	...
	protected void createPoints()
	{
		...
		for (float a = start; a <= end + step; a += step)
		{
			...
			float newX = cx + (MathUtils.cos((float) Math.toRadians(ang)) * radius1);
			float newY = cy + (MathUtils.sin((float) Math.toRadians(ang)) * radius2);
