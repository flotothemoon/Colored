...
public abstract strictfp class TerrainObject extends LevelObject implements Cloneable
{
	...
	public static TextureRegion getMainImage(TileType type)
	{
		try
		{
			return bufferedImages.containsKey(type) ? bufferedImages.get(type).get("20")[0][0] : null;
		}
		catch (Exception e)
		{
			throw new RuntimeException("Unable to get main image for type \"" + type + "\": " + e);
		}
	}
	...
	public void draw(Batch batch, Dimension dimension)
	{
		if (!this.hidden && this.isActive())
		{
			...
			else if (this.shouldDrawAsSolidBlock())
			{
				...
				{
					Renderer.setGrayScaleFactor(this.dimension == Dimension.COLORED ? 0.0f : this.level.getDimensionFactor(Dimension.MONOCHROME));
	...
	public void drawTextured(Dimension dimension, Batch batch)
	{
		...
		boolean identicalTwin = this.level.getType() == LevelType.NORMAL || this.shouldHandleAsShared() || this.hasMirror() && this.getMirror().getPosition().equals(this.position) && this.getMirror().getRotation() == this.rotation && this.innerType == ((TerrainObject) this.getMirror()).getInnerType();
		...
		{
			Renderer.setGrayScaleFactor(identicalTwin && !this.level.areDimensionOffset() ? this.level.getDimensionFactor(Dimension.MONOCHROME) : dimension == Dimension.COLORED ? 0.0f : 1.0f);
		}

		if (identicalTwin && !this.level.isDead())
		{
			color.a = 1.0f;
		}
		...
		else
		{
			if (this.reSimulationCooldown > 0 || LevelManager.isCheckpointing() && this.isMoveable() || this.level.getLevelType() == LevelType.FROZEN && this.isTimeEffected() && this.isMoveable() && (dimension == this.level.getForegroundDimension() || dimension == this.level.getBackgroundDimension() && this.level.isTransitioning()) && this.level.getControllingPlayer() != null && this.level.getControllingPlayer().isRawTimeEffected())
			{
				...
				this.drawTrailImage(this.image, trailImages, 10, dimension, ColorHelper.copy(color, ColorHelper.buffer), 0.4f, batch);
			}

			this.drawImage(this.image, dimension, this.rotation, this.position.x, this.position.y, color, batch);
		}

		this.postDraw(dimension, batch);
	}
	...
	protected void postDraw(Dimension dimension, Batch batch)
	{
