...
public class EntityIceSpike extends EntityProjectile
{
	...
	public boolean fall()
	{
		if (this.cooldown > 0 || !this.isSpawner)
		{
			return false;
		}
		...
		EntityIceSpike spike = (EntityIceSpike) this.copy(this.level);
		spike.fell = true;

		this.level.addEntity(spike);
	...
	{
		return ColorHelper.multiplyAlphaAndCopy(color, 1.0f - (float) this.cooldown / (float) this.totalCooldown * this.cooldown / this.totalCooldown);
	}
	...
	{
		return !(this.source instanceof EntityIceSpike) && super.canBeHarmedBy(entity);
	}
