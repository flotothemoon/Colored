...
public class GameLauncher extends Game
{
	...
	public static final String GAME_NAME_COLORED = "Colored" + (demoMode ? " Demo" : "");
	public static final String GAME_NAME_MONOCHROME = "Monochrome" + (demoMode ? " Demo" : "");

	private static BuildInfo buildInfo;

	private static Level levelContext;
	private static Dimension logoDimension;
	...
	private static GameStatus status, previousStatus;
	...
	public void create()
	{
		...
		Debug.log("\n\n[Started " + GAME_NAME_COLORED + " " + new SimpleDateFormat("dd/MM/yy HH:mm:ss").format(Calendar.getInstance().getTime()) + "]");
		...
		try
		{
			...
			Gdx.graphics.setTitle(GAME_NAME_COLORED + " (" + (developerMode ? "Developer Mode, " : "") + "Build " + buildInfo.name + ")");
			...
			cakePanel = new CakePanel(GameLauncher.getWidth() - 400, 10, 400, GameLauncher.getHeight() - 20);
	...
	public void update(int delta)
	{
		...
		{
			if (logoDimension != levelContext.getForegroundDimension())
			{
				logoDimension = levelContext.getForegroundDimension();

				updateLogoAndTitle();
			}

			if (Gdx.input.isKeyJustPressed(Input.Keys.F12)) // manual save
			{
	...
	public void render()
	{
		...
		try
		{
			...
			Color max = ColorHelper.applyAlphaAndCopy(Color.BLACK, 0.0f);

			int outlineSize = 12;

			sr.rect(0, 0, outlineSize, TARGET_HEIGHT, min, max, max, min);
			sr.rect(TARGET_WIDTH - outlineSize, 0, outlineSize, TARGET_HEIGHT, max, min, min, max);

			sr.rect(0, 0, TARGET_WIDTH, outlineSize, min, min, max, max);
			sr.rect(0, TARGET_HEIGHT - outlineSize, TARGET_WIDTH, outlineSize, max, max, min, min);
	...
	private static ByteBuffer[] loadIcons(String... iconPaths)
	{
		ByteBuffer[] icons = new ByteBuffer[iconPaths.length];

		for (int i = 0, n = iconPaths.length; i < n; i++)
		{
			Pixmap pixmap = new Pixmap(Gdx.files.internal("icon/" + iconPaths[i] + ".png"));
	...
	public static void updateLogoAndTitle()
	{
		String title = (levelContext.getForegroundDimension() == Dimension.COLORED ? GAME_NAME_COLORED : GAME_NAME_MONOCHROME) + " (" + (developerMode ? "Developer Mode, " : "") + "Build " + buildInfo.name + ")";
