...
public class MapManager
{
	...
	public static void initLevel(Vector2 spawnpoint, Level level)
	{
	...
	{
		updatePositions(level, x - level.getCameraOffset().x, y - level.getCameraOffset().y);
	}
	...
	public static boolean reSimulateAll(GameObject changedObject)
	{
		...
		Profiling.startSection("resimulate");
		...
		for (Tile tile : level.getTiles())
		{
			if (tile.getDimension() == Dimension.COLORED && tile.shouldReSimulate() && !tile.isShared())
			{
		...
		for (Entity entity : level.getEntities())
		{
			if (entity.getDimension() == Dimension.COLORED && entity.shouldReSimulate())
			{
				if (entity.checkPreReSimulation())
				{
					oldObjectPositions.put(entity, entity.getPosition().cpy());
		...
		Profiling.endSection("resimulate");
	...
	private static void drawTile(Tile tile, Dimension dimension, Batch batch)
	{
		...
		{
			tile.draw(batch, Dimension.MONOCHROME);
		}
	...
	public static void update(int delta)
	{
		...
		Profiling.endSection("camera");
	}

	public static void setFreeCamera()
	{
		freeCamera = true;
	}
