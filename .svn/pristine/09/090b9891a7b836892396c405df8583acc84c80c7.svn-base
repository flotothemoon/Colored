...
public class WrapperTexture extends GameObject implements IEmitterWrapper, Comparable<WrapperTexture>, ICopyable<WrapperTexture>
{
	...
	public void draw(Batch batch, Dimension dimension)
	{
		if (this.visible)
		{
			if (this.isParticleWrapper)
			{
				Profiling.startSection("particles");
				...
				if (this.emitter.hasActiveParticles() || this.emitter.hasMirror() && this.emitter.getMirroredEmitter().hasActiveParticles())
				{
				...
				Profiling.endSection("particles");
			}
			...
			{
				Profiling.startSection("image");

				Renderer.useSpriteBatch();

				this.drawImage((Sprite) this.image, dimension, batch);

				Profiling.endSection("image");
			}
		}
	}

	private void drawImage(Sprite image, Dimension dimension, Batch batch)
	{
		...
		Renderer.setGrayScaleFactor(dimension == Dimension.COLORED ? this.level.getDimensionFactor(Dimension.MONOCHROME) : this.level.getDimensionFactor(Dimension.COLORED));
		...
		if (this.foundTextures)
		{
			...
			if (image.isFlipX() ^ this.flippedHorizontally || image.isFlipY() ^ !this.flippedVertically)
			{
				image.flip(this.image.isFlipX() ^ this.flippedHorizontally, this.image.isFlipY() ^ !this.flippedVertically);
			}

			if (image.getRotation() != this.rotation)
			{
				image.setRotation(this.rotation);
			}

			image.setBounds(this.position.x + this.level.getXOffset(dimension), this.position.y + this.level.getYOffset(dimension), this.width, this.height);

			if (image.getOriginX() != image.getWidth() / 2 || image.getOriginY() != image.getHeight() / 2)
			{
			...
			else if (this.width != image.getRegionWidth() * this.scale || this.height != image.getRegionHeight() * this.scale)
			{
