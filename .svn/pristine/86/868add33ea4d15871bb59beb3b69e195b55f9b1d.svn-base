...
public class Particle extends Sprite implements IDeepCopyable<Particle>
{
	...
	private float initialWobbleFactor;

	private int wobbleInterval;
	private int totalWobbleInterval;
	...
	public void init(ParticleEmitter emitter, int lifetime)
	{
		...
		this.wobbleInterval = this.totalWobbleInterval = 0;
	...
	public void render(float baseAlpha, Dimension dimension, Batch batch)
	{
		...
		if (relXPos + halfSize >= 0 && relXPos - halfSize <= GameLauncher.getWidth() && relYPos + halfSize >= 0 && relYPos - halfSize <= GameLauncher.getHeight())
		{
			...
			{
				baseAlpha *= dimensionColor.a;
			}
			else
			{
				baseAlpha *= level.getControlAlpha();
			}
			...
			this.emitter.getSystem().renderedParticleCount++;
		}
	}
	...
	public float calculateWobbleFactor()
	{
		return Transitions.getFactor(Math.abs((float) this.wobbleInterval / (float) this.totalWobbleInterval - 0.5f) * 2.0f);
	}
	...
	public int getWobbleInterval()
	{
		return this.wobbleInterval;
	}

	public int getTotalWobbleInterval()
	{
		return this.totalWobbleInterval;
	}

	public void setWobbleInterval(int wobbleInterval)
	{
		this.wobbleInterval = wobbleInterval;
	}

	public void setTotalWobbleInterval(int totalWobbleInterval)
	{
		this.totalWobbleInterval = totalWobbleInterval;
	}

	public float getInitialWobbleFactor()
	{
		return this.initialWobbleFactor;
	}

	public void setInitialWobbleFactor(float initialWobbleFactor)
	{
		this.initialWobbleFactor = initialWobbleFactor;
	}
}