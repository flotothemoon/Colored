...
public class EndingStoryHandler extends CustomStoryHandler
{
	...
	public void onLeverPulled(TileLever lever, EntityPlayer interactor, boolean realInput)
	{
		...
		{
			if (interactor.getDimension() == Dimension.COLORED)
			{
				...
				EntityScientist selfAwareShadowPlayer = new EntityScientist(shadowPlayer.getPosition().cpy(), level, Dimension.MONOCHROME, false);
			...
			else if (interactor.getDimension() == Dimension.MONOCHROME)
			{
	...
	{			
		...
		if (!mirroredPlayer.isRemoved() && mirroredPlayer.getDistanceFromSpawnpoint().len() > 30.0f && player.getHitbox().intersects(mirroredPlayer.getHitbox()))
		{			
			player.onDeath(null);
		}
