...
public class EntityFileHandler
{
	...
	public void write(List<Entity> entities)
	{
		...
		{
			if (entity.getDimension() == Dimension.COLORED && entity.isPersistent())
			{
				line[0] = entity.getType() + "";
				line[1] = (entity.getX() - MapEditor.getLevel().getCameraOffset().x) / DisplayConstants.TILE_SIZE + "";
				line[2] = (entity.getY() - MapEditor.getLevel().getCameraOffset().y) / DisplayConstants.TILE_SIZE + "";
	...
	public ArrayList<Entity> read(Level level)
	{
		...
		try
		{
			...
			{
				Entity entity = Entity.createEntity(EntityType.getTypeByNameTag(line[0]), new Vector2(Float.parseFloat(line[1]) * DisplayConstants.TILE_SIZE, Float.parseFloat(line[2]) * DisplayConstants.TILE_SIZE), level, Dimension.getDimension(Integer.parseInt(line[3])), true);
