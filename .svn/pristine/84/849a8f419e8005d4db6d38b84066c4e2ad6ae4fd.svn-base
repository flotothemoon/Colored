...
public abstract class LevelObject extends GameObject implements IDeepCopyable<LevelObject>
{
	...
	protected Vector2 attractionVelocity = new Vector2(0.0f, 0.0f);
	...
	protected void enableStencilDrawing(Batch batch)
	{
	...
	protected void enableStencilCheck(Batch batch)
	{
		batch.flush();
	...
	protected void disableStencilCheck(Batch batch)
	{
		batch.flush();
	...
	protected void drawEmitters(boolean foreground, boolean stencilled, boolean ignoreStencil, Dimension dimension, Batch batch)
	{
		if (this.particleEffects != null)
		{
			for (EmitterType emitterType : this.particleEffects.keySet())
			{
				if (emitterType.foreground == foreground)
				{
					for (ParticleEmitter emitter : this.particleEffects.get(emitterType))
					{
						if (ignoreStencil || stencilled == emitter.isStencilled())
						{
	...
	public boolean hasActiveEmitters()
	{
		...
		for (HashSet<ParticleEmitter> emitterSet : this.particleEffects.values())
		{
			...
			{
				if (emitter.isEnabled() && (emitter.hasActiveParticles() || emitter.hasLimitedLength() && !emitter.isCompleted()))
				{
					return true;
				}
			}
	...
	{
		return this.addEmitter(type, dimension, null);
	}

	public ParticleEmitter addEmitter(EmitterType type, Dimension dimension, ICustomEmitterInit customEmitterInit)
	{
		return this.addEmitter(type, dimension, type.getMaxParticles(type, (int) this.hitbox.getWidth(), (int) this.hitbox.getHeight()), customEmitterInit);
	}
	...
	protected ParticleEmitter addEmitter(EmitterType type, Dimension dimension, int width, int height, ICustomEmitterInit customEmitterInit)
	{
		return this.addEmitter(type, dimension, null, width, height, type.getMaxParticles(type, width, height), false, 0, customEmitterInit);
	}

	protected ParticleEmitter addEmitter(EmitterType type, Dimension dimension, int maxParticles, ICustomEmitterInit customEmitterInit)
	{
		return this.addEmitter(type, dimension, null, (int) this.hitbox.getRawWidth(), (int) this.hitbox.getRawHeight(), maxParticles, false, 0, customEmitterInit);
	}

	protected ParticleEmitter addEmitter(EmitterType type, Dimension dimension, final Vector2 forcedPosition, int width, int height, int maxParticles, ICustomEmitterInit customEmitterInit)
	{
		return this.addEmitter(type, dimension, forcedPosition, width, height, maxParticles, false, 0, customEmitterInit);
	}

	protected ParticleEmitter addEmitter(EmitterType type, Dimension dimension, final Vector2 forcedPosition, int width, int height, int maxParticles, final boolean forceRotation, final int forcedRotation, final ICustomEmitterInit customEmitterInit)
	{
		if (this.particleEffects == null)
		{
		...
		this.particleEffects.get(type).add(emitter = this.level.addParticleEmitter(dimension, new EmitterWrapperConfiguration(this, type, useColor ? color : type.startTint, useColor ? color : type.endTint, this.hitbox, width, height, this.rotation, maxParticles, this.image, this.image != null)
		{
			...
			{
				if (customEmitterInit != null)
				{
					customEmitterInit.applyCustomInit(this, emitter);
				}
			}
	...
	{
		if (this.particleEffects != null && this.particleEffects.containsKey(type))
		{
			Iterator<ParticleEmitter> iterator = this.particleEffects.get(type).iterator();
