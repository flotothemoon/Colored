...
public class SettingsMenu extends Menu
{
	...
	private static final int SOUND_SLIDER_PLAY_INTERVAL = 1000;

	private long lastSoundPlayerTime;
	private MenuItemSlider soundVolumeSlider;
	private MenuItemSlider musicVolumeSlider;
	...
	public SettingsMenu(float yOffset, float padding)
	{
		...
		for (final MenuItem item : this.options)
		{
			...
			{
				this.soundVolumeSlider = (MenuItemSlider) item;
			...
			{
				this.musicVolumeSlider = (MenuItemSlider) item;
				((MenuItemSlider) item).setListener(new IMenuItemSliderListener()
				{
					@Override
					public void onFactorChanged(float selectedRange)
	...
	public void customUpdate(int delta)
	{
		...
		{
			if (this.options[this.selectedOption] == this.soundVolumeSlider)
			{
				if (System.currentTimeMillis() - this.lastSoundPlayerTime >= SOUND_SLIDER_PLAY_INTERVAL)
				{
					MenuHandler.getSoundManager().play(SoundType.randomSoundFromAll());

					this.lastSoundPlayerTime = System.currentTimeMillis();
				}
			}
			else if (this.options[this.selectedOption] == this.musicVolumeSlider)
			{

			}
		}

		this.options[2].setItem(GameLauncher.isFullscreenMode() ? MenuOption.SWITCH_TO_WINDOWED.name : MenuOption.SWITCH_TO_FULLSCREEN.name);
	}
