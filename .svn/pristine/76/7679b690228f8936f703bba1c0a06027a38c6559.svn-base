...
public enum EmitterType
{
	...
	LIGHT_CAGE_CLOSING("cage_closing"),

	TIME_TRAVEL_CHANGED("tt_changed")
	{
		{
			this.foreground = false;
			this.limitLifetimeToParticles = true;
		}
	},
	...
	CREATURE_DEATH_BG("creature_death_bg")
	{
		{
			this.foreground = false;
			this.limitLifetimeToParticles = true;
	...
	{
		{
			...
			this.renderMode = ParticleEmitter.RENDER_PARTIAL;
			this.rotateAccelerations = false;
			this.limitLifetimeToParticles = true;
		}
	},
	...
	DOORSPARKLE("doorsparkle"), DOORSPIRAL("doorspiral"),

	HOLE_ATTRACT("hole_attract"),

	SCROLLBAR("scrollbar"), SELECTION_BOX("selection_box"),

	SLIDER_FULL("slider_full"), SLIDER_SELECTED("slider_selected"),

	CREATURE_PRESHOOT("creature_preshoot")
	{
		{
			this.foreground = false;
			this.limitLifetimeToParticles = true;
	...
	public IntegerRange wobbleInterval;
	...
	public boolean circularInversed;
	public boolean wobbleFactor;
	...
	{
		this(spritePath, tint, lifeTime, spawnInterval, spawnCount, initialLife, initialSize, xOffset, yOffset, velocityX, velocityY, startRotation, rotationVelocity, growthFactor, gravityFactor, windFactor, startAlpha, fadeInTime, fadeOutTime, shouldFadeIn, shouldFadeOut, limitToWrapper, tightBounce, shouldBounceOutOfArea, hasLimitedLifetime, scaleWithSize, originalWidth, originalHeight);
	...
	private void readFromTemplate(String templateName)
	{
		...
		this.wobbleInterval = emitter.getWobbleInterval();
		...
		this.circularInversed = emitter.isCircularInversed();
		this.additiveMode = emitter.isAdditive();
		this.tightBounce = emitter.isTightBounce();
		this.wobbleFactor = emitter.isWobbleMode();
	...
	{
		if (this.scaleWithWidth)
		{
			if (this.scaleWithHeight)
			{
